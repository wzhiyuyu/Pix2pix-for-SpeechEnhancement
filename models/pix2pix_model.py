...
class UNetSkipConnectionBlock(nn.Module):
    ...
    def forward(self, ipt):
        if self.outer_most:
            # 第一次调用时，从最外层开始，输入为 c x h x w
            # 最外层：卷积() + Norm() + [子模块, ..., Norm] + ReLU + 卷积 + Tanh
            return self.model(ipt)
        else:
            return torch.cat([ipt, self.model(ipt)], 1)
...
class Discriminator(nn.Module):
    def __init__(self, n_filters_first=64, use_bn_in_last_layer=True):
        ...
        network = [
            nn.Conv2d(2, n_filters_first, 4, stride=2, padding=1),
            nn.LeakyReLU(0.2, inplace=True),

            nn.Conv2d(n_filters_first, n_filters_first * 2, 4, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(n_filters_first * 2),
            nn.LeakyReLU(0.2, inplace=True),

            nn.Conv2d(n_filters_first * 2, n_filters_first * 4, 4, stride=2, padding=1, bias=False),
